...
public class KanopusExcel {
    ...
    public KanopusExcel(int rowAccessWindowSize, boolean compressTmpFiles) {
        this.wb = new SXSSFWorkbook(new XSSFWorkbook(), rowAccessWindowSize, compressTmpFiles, true);
        this.factory = this.wb.getCreationHelper();
        createStyles(this.wb);
    }
     ...
     */
    private static void createStyles(SXSSFWorkbook workbook) {
        if (styles.isEmpty()) {
    ...
    private static CellStyle createStyle(SXSSFWorkbook workbook, boolean bold, short fontColor, short backgroundColor) {
